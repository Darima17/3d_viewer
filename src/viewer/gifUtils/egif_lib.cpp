...
EGifGetGifVersion(GifFileType *GifFile)
{
    ...
    if (Private->gif89)
	return (char*)GIF89_STAMP;
    else
	return (char*)GIF87_STAMP;
}
...
EGifPutComment(GifFileType *GifFile, const char *Comment)
{
    ...
    if (length <= 255) {
        return EGifPutExtension(GifFile, COMMENT_EXT_FUNC_CODE,
                                length, (const unsigned char*)Comment);
    } else {
        buf = (char *)Comment;
        ...
        while (length > 255) {
            if (EGifPutExtensionBlock(GifFile, 255, (const unsigned char*)buf) == GIF_ERROR) {
                return GIF_ERROR;
            }
            buf = buf + 255;
            length -= 255;
        ...
        if (length > 0) {
            if (EGifPutExtensionBlock(GifFile, length, (const unsigned char*)buf) == GIF_ERROR) {
                return GIF_ERROR;
            }
        }
